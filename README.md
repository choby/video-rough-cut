# 视频粗剪 skill
本 skill 适用于将长视频粗剪为多个精华片段， 本项目分为 3 个 skill：
- 转录：将视频文件抽取的音频文件上传到阿里 OSS 存储桶中，调用火山引擎语音识别 API 进行转录，生成口播逐字稿。
- 提取关键帧：根据口播逐字稿，提取出精华片段的时间戳。这一步需要先将口播逐字稿交由人工或者大模型处理，提取出你想要的精华内容，有了精华内容后， 再通过改技能提取精华片段时间戳。
- 剪辑：根据提取出的时间戳，剪辑出精华片段的视频文件。
------
# 前置条件
